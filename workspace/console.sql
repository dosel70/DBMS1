...
/*
    ...
    회원      사무실         회의실         파트타임        이용시간
    번호 pk   번호 pk       번호 pk        번호 pk          번호pk
                          사무실fk        회의실fk        회원정보 fk
                                                        회의실 fk
...
create table tbl_member(
    ...
    constraint check_level check ( user_rank in ('기본','단골') )
);
...
/*
    ...
    아이      부모님     체험학습     체험학습이미지   체험 등록
    번호pk      번호pk      번호pk       번호pk         번호pk
   아이번호fk(부모님 번호)                             이미지번호 fk(체험학습번호)  등록번호fk(아이번호)
*/

create table tbl_parent ( # 부모테이블 unique >> 주소,번호
    id bigint primary key,
    name varchar(255) not null,
    age bigint not null,
    address varchar(255) not null unique ,
    number bigint(255) not null unique,
...
create table tbl_img (
    ...
    fild_trip_id bigint not null,
    constraint fk_trip_img foreign key (fild_trip_id)
                     references tbl_trip(id)

);

create table tbl_register( /* 체험 학습 등록 >> 어린이 */
    id bigint primary key, /* 주석 */
    child_id bigint not null,
    constraint fk_register_child foreign key (child_id)
                         references tbl_child(id),
    constraint fk_register_trip foreign key (child_id)
                         references tbl_trip(id)
...
/*
    ...
    기업(광고주)      광고                         대카테고리       중카테고리               소카테고리
    번호pk            번호pk                      번호pk          번호 pk                   번호 pk
                    광고번호fk(소카테고리id)                       중카테고리번호fk(대카테고리id)  소카테고리번호fk(중카테고리id)
*/
...
/*
    ...
    # 논리 모델링
    음료수             당첨자             당첨번호            당첨 상품              배송테이블
    번호pk              번호pk              번호pk          번호pk                  번호pk
                                        당첨번호id fk(당첨자)  당첨상품id fk(당첨번호)  배송테이블id fk(당첨자)
...
create table tbl_lottery_number(
    ...
    constraint fk_lottery_person foreign key(lottery_id)
...
create table tbl_price(
    ...
    constraint fk_price_lottery foreign key(price_id)
...
create table tbl_deliver(
    ...
    constraint check_delivery_status check ( status in ('배송중' , '배송완료') ),
    constraint fk_deliver_person foreign key(delivery)
                        references tbl_person(id)
...
/*
    ...
    기업      사용자     상품              카드              결제
pk  번호      번호      번호              번호              번호
fk                                      정보(사용자id)      정보(회원id,카드id)
    */
create table tbl_economy ( # 기업 정보
    id bigint auto_increment primary key ,
    name varchar(255) not null,
    address varchar(255) not null,
...
create table tbl_price (# 상품
    id bigint auto_increment primary key ,
    name varchar(255) not null,
    price_price int default 0,
    price_balance bigint default 0,
    economy_id bigint not null,
    constraint fk_price_economy foreign key (economy_id)
                       references tbl_economy(id)

);
create table tbl_card ( # 카드사 정보
    id bigint auto_increment primary key ,
    card_number varchar(255) not null ,
    card_office varchar(255) not null,
    user_info varchar(255) not null,
    constraint fk_card_user foreign key(user_info)
...
create table tbl_pay ( # 결제 정보
    id bigint ,
    create_date date default  (current_date),

    primary key (id,create_date), /* 조합키 사용 */

    user_info varchar(255) not null,
    card_info varchar(255) not null,
    constraint fk_pay_user foreign key(user_info)
                 references tbl_user(id),
    constraint fk_pay_card foreign key(card_info)
